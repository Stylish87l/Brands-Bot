...
const MascotVariations: React.FC<MascotVariationsProps> = ({ variations, isLoading, error, onSelect }) => {
  if (isLoading) {
    return (
      <div className="mt-4 flex items-center justify-center p-4 bg-gray-200/50 dark:bg-gray-700/50 rounded-md transition-colors">
        ...
        <span className="text-sm text-gray-700 dark:text-gray-300">Generating mascot ideas...</span>
      </div>
    );
  }
  ...
  return (
    <div className="mt-4 space-y-3">
      <h4 className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Select a Mascot</h4>
      <div className="grid grid-cols-2 gap-3">
        {variations.map((dataUrl, index) => (
          <button
            ...
            aria-label={`Select mascot variation ${index + 1}`}
          >
            <img src={dataUrl} alt={`Mascot Variation ${index + 1}`} className="max-w-full max-h-full object-contain" />
          </button>
        ))}
      </div>
    </div>
...
export default MascotVariations;